import { BillPayRequestForm } from "@/components/dashboard/BillPayRequestForm";
...
export default async function BillPayPage() {
  ...
  const [accounts, billPayments] = await Promise.all([
    ...
    BillPayRequest.find({ userId: session.userId }).sort({ createdAt: -1 }).limit(25).lean()
  ]);
      ...
      <section className="border-b border-bank-line bg-bank-mist">
        <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:py-16">
          ...
          <h1 className="mt-5 max-w-5xl text-4xl font-semibold tracking-[-0.045em] text-ink-950 sm:text-5xl">
            Submit a bill-pay request for review.
          </h1>

          <p className="mt-5 max-w-3xl text-lg leading-8 text-bank-steel">
            This prototype captures bill-pay requests only. It does not send payments,
            connect to billers, debit accounts, or change balances.
          </p>
        </div>
      </section>
